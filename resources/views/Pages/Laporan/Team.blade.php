...
<div class="card">
   <div class="card-header">
      <h3 class="card-title">Data Laporan Team</h3>
   </div>
   <div class="card-body">
      <table id="datatables" class="table table-bordered table-striped">
         ...
         <tbody>
            ...
            <tr>
               ...
               <td>{{ $team->judul }}</td>
               <td>{{ $team->jenis_laporan }}</td>
               <td>{{ $team->tanggal }}</td>
               <td>
                  <a href="{{ route('laporan.edit', $team->id) }}" class="btn btn-sm btn-warning">
                     <i class="fas fa-tools" style="color: #ffffff"></i>
                  </a>
                  <a href="{{ route('laporan.download', $team->id) }}" class="btn btn-sm btn-info" title="Download PDF">
                     <i class="fas fa-download"></i>
                  </a>
                  <form action="{{ route('laporan.delete', $team->id) }}" method="POST" class="d-inline">
                     @method('DELETE')
                     @csrf
